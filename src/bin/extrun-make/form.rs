...
impl Form {
    ...
    pub fn paste_hint(&self) -> &'static str {
        match self.ext_style {
            ExtStyle::Section => "extrun-config.txt の末尾に貼り付けてください",
            // 符号なし＝完全置換なので、貼り先のセクションの指定は無視される
            ExtStyle::PerItem => "どのセクションの下に貼っても同じように動きます",
        }
    }
}
...
mod tests {
    ...
    fn 貼り先の案内は書き方で変わる() {
        let mut form = 見本();
        assert!(form.paste_hint().contains("末尾"));
        form.ext_style = ExtStyle::PerItem;
        assert!(form.paste_hint().contains("どのセクション"));
    }
}
