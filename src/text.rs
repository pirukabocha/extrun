...
/// `&` や `+` を含むフォルダ名をそのまま書けるよう、対象は 4 つに絞る。
///
/// **`@` は潰さない。** ここは `@apps\7-Zip\7z.exe` のように**別名を書く欄**で、
/// `^@` にすると別名として解決されなくなる（設定づくりの「別名を挿入」が
/// そのまま死ぬ）。引き換えに、パスに素の `@` が含まれる場合は書いた人が
/// `^@` と書く必要があるが、**未定義の別名としてエラーになる**ので黙って
/// 壊れることはない。`@` を含む実在のプログラムのパスはまず無い。
///
/// `escape_name` が `@` を潰すのと食い違うのは意図的で、**名前欄に別名を
/// 書く意味がほとんど無い**のに対し、パス欄はそれが主な使い道だから。
///
/// **`%` も対象外**。`%NAME%` の展開はパス欄の機能なので、潰すと
...
mod tests {
    ...
    /// パス欄は別名を書く欄なので `@` をそのまま通す
    #[test]
    fn パス欄の別名は潰さない() {
        assert_eq!(escape_path(r"@apps\7-Zip\7z.exe"), r"@apps\7-Zip\7z.exe");
    }
