...
長辺を指定して縮小
 | @magick
 | $p -resize $?{長辺のピクセル数=1280}x$?{長辺のピクセル数=1280}^> $-p_$?{長辺のピクセル数=1280}.png

品質を指定して JPEG に変換 [-.jpg -.jpeg]
 | @magick
 | $p -quality $?{品質 1-100=85} $-p_q$?{品質 1-100=85}.jpg

IrfanView で長辺を指定して縮小
 | @irfan
 | $p /resize_long=$?{長辺のピクセル数=1280} /aspectratio /resample /convert=$-p_$?{長辺のピクセル数=1280}.jpg
```

> **使用**: `$?{...}`（入力欄）/ `$-p` / `[-.jpg]` / `^>`（エスケープした `>`）
> **同じ内容の `$?{...}` は 1 回しか聞かれません。** 上の例では入力した値を、変換の指定と出力ファイル名の両方に使っています。値ごとに項目を並べる代わりに 1 項目で済み、しかも出力名に値が残るので後から見分けられます。
