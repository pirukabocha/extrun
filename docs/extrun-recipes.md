...
**このレシピ集の項目名にはアクセスキー（`&`）を入れていません。** キーは同じメニューの中で重ならないことが条件なので、どのレシピを採用するかで正解が変わります。貼り付けたあと、自分の設定に合わせて `圧縮 (&Z)` のように足してください（→ [extrun-config-format.md](extrun-config-format.md#アクセスキー)）。重複は `--check` が教えてくれます。
...
形式を変換
> PNG にする  [-.png +.svg +.heic] | @magick | $p $-p.png
...
> **使用**: `[-.png]` `[-.jpg -.jpeg]`（引き算は複数書ける）/ `[+.svg]`（足し算）
...
+ タイル状に並べて 1 枚にする
 | @magick
 | montage $p -tile $?int{横に並べる数=4}x -geometry +4+4 $d\contact_$?int{横に並べる数=4}x.png
```
...
長辺を指定して縮小
 | @magick
 | $p -resize $?int{長辺のピクセル数=1280}x$?int{長辺のピクセル数=1280}^> $-p_$?int{長辺のピクセル数=1280}.png

品質を指定して JPEG に変換 [-.jpg -.jpeg]
 | @magick
 | $p -quality $?int{品質 1-100=85} $-p_q$?int{品質 1-100=85}.jpg

名前を付けて PNG に変換 [-.png]
 | @magick
 | $p $d\$?name{新しい名前=$a}.png

IrfanView で長辺を指定して縮小
 | @irfan
 | $p /resize_long=$?int{長辺のピクセル数=1280} /aspectratio /resample /convert=$-p_$?int{長辺のピクセル数=1280}.jpg
```

> **使用**: `$?int{...}` `$?name{...}`（入力欄と決まり）/ `$-p` / `[-.jpg]` / `^>`（エスケープした `>`）
> **同じ書き方の `$?{...}` は 1 回しか聞かれません。** 上の例では入力した値を、変換の指定と出力ファイル名の両方に使っています。値ごとに項目を並べる代わりに 1 項目で済み、しかも出力名に値が残るので後から見分けられます。
> 既定値（`=` の右）は選択された状態で入るので、いつもの値でよければ Enter だけで確定できます。キャンセルすると何も実行されません。
> `$?` と `{` のあいだの `int` は「整数だけを受け付ける」指定です。`1280px` や全角の `１２８０` は弾かれ、理由を添えて聞き直されます。**画像アプリはこの手の値を渡されても分かりにくいエラーで落ちるだけで、しかも ExtRun は起動して即終了するのでその画面は残りません。** 入口で止めた方が原因が分かります。
> `$?name{...}` はファイル名に使えない文字（`\ / : * ? " < > |`）や予約された名前（`CON` など）、長すぎる名前を弾きます。
> `^>` は ImageMagick の「指定より大きいときだけ縮小する」指定です。`>` は行頭ではサブメニューの印なので、引数の中で使うときは `^` でエスケープします。
...
バックアップ
> 日付つき         | @7zg | a -t7z $-p_$t{yyyyMMdd}.7z $p
> 日時つき（秒まで）| @7zg | a -t7z $-p_$t{yyyyMMdd-HHmmss}.7z $p
> + 今日のぶんをまとめて | @7zg | a -t7z $d\$f_$t{yyyyMMdd}.7z $p
> ---
# 日付のフォルダに振り分ける。7-Zip は書庫の親フォルダが無ければ作ります
> 日付フォルダに保存 | @7zg | a -t7z $d\backup\$t{yyyy-MM}\$a_$t{dd(ddd)}.7z $p
```

> **使用**: `$t{...}`（日時）/ `$-p` / `$f` / `+`
> `$t{...}` は**項目を選んだ時点**の日時に置き換わります。1 回の実行の中では対象をまたいで同じ値になるので、複数のファイルを選んで「日時つき（秒まで）」を実行しても、秒がずれて名前がばらけることはありません。
> 書式は `yyyy`（年）`MM`（月）`dd`（日）`HH`（時）`mm`（分）`ss`（秒）で、**`ddd` は日本語の曜日**（`土`）、`EEE` は英語の曜日（`Sat`）です。`d` は文字数で意味が変わり、`dd` は日・`ddd` は曜日なので注意してください。
> 中括弧の中に書式以外の英字は書けません（`$t{yyyyMMdd_bak}` ではなく `$t{yyyyMMdd}_bak`）。書き間違いは `--check` がエラーとして報告します。
...
1 つずつ開いて直す | @code | --wait -n $p
 :wait
```

> **使用**: `+` / `:wait` / `--wait`
> 最後の項目は「1 つ開く → 手で直す → 閉じると次の 1 つが開く」という進み方になります。10 個選んでも一度に開くのは 1 つだけです。
>
> **`:wait` と VS Code の `--wait` は両方要ります。** `:wait` は ExtRun 側の「前の 1 つが終わるまで次を起動しない」指定で、`--wait` は VS Code 側の「タブを閉じるまでプロセスを終了しない」指定です。VS Code は単一インスタンス型なので、`--wait` が無いと**起動したプロセスがすぐ終了して既存のウィンドウに引き継ぐ**ため、ExtRun からは「もう終わった」ように見えて全部が一度に開きます。
>
> 単一インスタンス型のアプリで `:wait` が効かないときは、同じように「終了まで待つ」オプションが無いか探してください（無ければ `:delay` で間隔を空けるほかありません）。詳しくは [`:wait` の仕様](extrun-config-format.md#1-つずつ順に実行)を参照。
...
**このときは `--at window --select-first` を付けるのがおすすめです。** ホットキーから呼ぶとマウスカーソルがどこにあるか分からず（別のモニタにあることすらあります）、既定のカーソル位置では思わぬ場所にメニューが出ます。`--at window` なら前面ウィンドウの中央に、`--select-first` なら先頭項目を選択した状態で開くので、**マウスに触れずに操作を完結できます**。設定ファイルは右クリック用の既定のままで構いません（引数のほうが優先されます）。
...
SendToExtRun() {
    ...
    ; ホットキーから呼ぶのでカーソル位置は当てにならない。
    ; ウィンドウ中央に出し、先頭項目を選択した状態で開く。
    try
        Run '"' ExtRunPath '" --at window --select-first' args
    catch as err
