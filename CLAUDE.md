...
- **`:wait` は「起動したプロセスが終わるまで次を起動しない」**: 待ちの実体は `:delay` と同じ進行状況ダイアログのモーダルループで、タイマーが**起動の合図ではなく様子見の合図**になる（`progress::Step::Busy` を返せば番号が進まない）。終了の判定は `menu::launch_all` が持ち、`progress.rs` は起動も終了判定も知らない（組み立てを `resolve_invocations` の 1 か所に集めるという約束のため）。
...
- `menu.rs` の `:delay` / `:wait` — 進行状況を出すしきい値（`shows_progress`）、中止したときに残るパスの範囲（`remaining_paths`）、`launch_in_order` が `Busy` と `Stop` をどう扱うか。**終了の判定は実プロセスで確かめる**（`cmd /c ping` を起動して `Running::alive` の移り変わりを見る）。ダイアログ経由の逐次起動は `#[ignore]` 付きで、**かかった時間**で重なっていないことを判定する（3 プロセスで 2 秒以上。重ねて起動していれば 1 つぶんで終わる）。
